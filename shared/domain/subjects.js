...
export const SCIENCE_ELECTIVE_KEYS = ['BIOLOGY', 'CHEMISTRY', 'PHYSICS', 'ICT', 'MATHEMATICS_M1', 'MATHEMATICS_M2'];
export const BUSINESS_ELECTIVE_KEYS = ['ECONOMICS', 'BAFS'];
export const ARTS_SPORTS_ELECTIVE_KEYS = ['MUSIC', 'VISUAL_ARTS', 'PHYSICAL_EDUCATION'];
// ===== 辅助函数 =====
/**
 * 根据 key 获取科目定义
 */
 ...
 * 根据 key 获取科目显示名称
 */
export function getSubjectDisplayName(key, lang) {
 ...
 * 检查 key 是否为有效科目
 */
export function isValidSubjectKey(key) {
    return ALL_SUBJECT_KEYS.includes(key);
}
/**
 * 检查 key 是否为核心科目
 */
export function isCoreSubject(key) {
    return CORE_SUBJECT_KEYS.includes(key);
}
/**
 * 检查 key 是否为选修科目
 */
export function isElectiveSubject(key) {
    return ELECTIVE_SUBJECT_KEYS.includes(key);
}
/**
 * 检查科目是否使用 Pass/Fail 成绩体系
 */
export function hasPassFailGrading(key) {
    const subject = getSubjectByKey(key);
 ...
 * 检查是否为理科选修
 */
export function isScienceElective(key) {
    return SCIENCE_ELECTIVE_KEYS.includes(key);
}
/**
 * 检查是否为商科选修
 */
export function isBusinessElective(key) {
    return BUSINESS_ELECTIVE_KEYS.includes(key);
}
/**
 * 检查是否为艺术/体育类选修
 */
export function isArtsSportsElective(key) {
    return ARTS_SPORTS_ELECTIVE_KEYS.includes(key);
}
/**
 * 生成 Select 组件的 options（根据语言）
 */
export function getSubjectOptions(lang, category) {
 ...
 */
export function getElectiveAnalysisNotes(keys, lang) {
    const notes = [];
    ...
    if (hasScience)
        notes.push(NOTES.science[lang]);
    if (hasBusiness)
        notes.push(NOTES.business[lang]);
    if (hasArtsSports)
        notes.push(NOTES.artsSports[lang]);
    return notes;
}
// ===== 向后兼容导出（过渡期使用，后续可移除） =====
