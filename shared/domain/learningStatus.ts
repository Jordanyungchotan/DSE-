...
export const LEARNING_STATUS_OPTIONS: LearningStatusOption[] = [
  {
...
export const RANK_POSITION_OPTIONS: RankPositionOption[] = [
  {
...
export const SCORE_SOURCE_OPTIONS: ScoreSourceOption[] = [
  {
 ...
 * 检查是否为有效的学习状态
 */
export function isValidLearningStatus(status: string): status is LearningStatus {
  return Object.values(LEARNING_STATUS).includes(status as LearningStatus);
}

/**
 * 检查是否为有效的校内位置
 */
export function isValidRankPosition(position: string): position is RankPosition {
  return Object.values(RANK_POSITION).includes(position as RankPosition);
}

/**
 * 检查是否为有效的成绩来源
 */
export function isValidScoreSource(source: string): source is ScoreSource {
  return Object.values(SCORE_SOURCE).includes(source as ScoreSource);
}

/**
 * 获取学习状态显示文本
 */
export function getLearningStatusLabel(status: LearningStatus, lang: LanguageCode): string {
 ...
 * 获取校内位置显示文本
 */
export function getRankPositionLabel(position: RankPosition, lang: LanguageCode): string {
 ...
 * 获取成绩来源显示文本
 */
export function getScoreSourceLabel(source: ScoreSource, lang: LanguageCode): string {
