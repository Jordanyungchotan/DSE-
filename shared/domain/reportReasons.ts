 ...
 * 获取插班分析原因文本
 */
export function getTransferReasonText(key: keyof typeof TRANSFER_REASONS, lang: LanguageCode): string {
 ...
 * 获取大学分析原因文本
 */
export function getUniversityReasonText(key: keyof typeof UNIVERSITY_REASONS, lang: LanguageCode): string {
 ...
 * 获取建议文本
 */
export function getAdviceText(key: keyof typeof ADVICE_TEXT, lang: LanguageCode): string {
 ...
 * 批量获取原因文本
 */
export function getReasonTexts(
  keys: string[], 
  lang: LanguageCode, 
  type: 'transfer' | 'university' = 'transfer'
): string[] {
  const reasons = type === 'transfer' ? TRANSFER_REASONS : UNIVERSITY_REASONS;
  return keys.map(key => (reasons as Record<string, I18nText>)[key]?.[lang] ?? key);
}
