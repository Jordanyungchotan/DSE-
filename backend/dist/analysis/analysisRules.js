import { CIVICS_STATUS, isScienceElective, isBusinessElective, isArtsSportsElective, } from "../../../shared/domain";
// ===== 学习状态分析规则（插班分析专用）=====
/**
 * 学习状态对应的风险等级和分析结果
 *
 ...
 * - bottom: 提高风险
 */
export const RANK_POSITION_ADJUSTMENTS = {
    top: {
        riskAdjustment: -1,
        note: {
 ...
 * 根据学习状态和校内位置计算最终风险等级
 */
export function calculateRiskLevel(status, rankPosition) {
    const baseRiskLevels = { low: 0, medium: 1, high: 2 };
    const reverseRiskLevels = ['low', 'medium', 'high'];
 ...
 * 获取选修科目类别（基于 key）
 */
export function getElectiveCategory(subjectKey) {
 ...
 */
export function getElectiveNotes(subjectKeys, lang = 'zh-CN') {
    const notes = [];
 ...
 * - E: <30 (全部 weak)
 */
export function calculateOverallFeasibility(statuses) {
...
//# sourceMappingURL=analysisRules.js.map