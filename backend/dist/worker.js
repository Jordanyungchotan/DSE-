...
// 检查是否是允许的来源
function isAllowedOrigin(origin, envOrigin) {
    if (!origin)
        return false;
...
// 错误响应
const errorResponse = (message, status = 500, origin = '*') => {
...
// 数值相等性判断
function isNumericEqual(a, b, precision = 0.001) {
    if (b === 0)
        return Math.abs(a) < precision;
    return Math.abs(a - b) / Math.abs(b) < precision;
}
// 提取方程解
function extractEquationSolution(equation) {
...
// 检测题目类型（从题目文本中推断）
function detectQuestionSubType(questionText) {
...
// 检查是否为纯数字答案
function isNumericOnly(answer) {
    return /^\s*[-+]?\d+(?:\.\d+)?\s*$/.test(answer);
}
// 智能答案匹配主函数 (增强版)
function intelligentAnswerMatch(userAnswer, expectedAnswer, questionType, options, questionText) {
    ...
    if (questionType === 'multiple_choice') {
        ...
        // 情况3: 用户答案是完整选项文本
            ...
            const matchedIndex = options.findIndex(opt => normalizeAnswer(opt).toLowerCase() === normalizedUser.toLowerCase());
            if (matchedIndex >= 0) {
        ...
        // 情况3: 正确答案是完整选项文本
            ...
            const matchedIndex = options.findIndex(opt => normalizeAnswer(opt).toLowerCase() === normalizeAnswer(expectedStr).toLowerCase() ||
        ...
        if (options && options.length > 0 && !userChoice) {
            const userIndex = options.findIndex(opt => normalizeAnswer(opt).toLowerCase() === normalizedUser.toLowerCase());
            if (userIndex >= 0) {
...
function calculateGradeEquivalent(score, targetGrade) {
...
function generateBasicRecommendations(weaknessPoints, subject) {
...
// 详细能力雷达图计算
function calculateDetailedAbilityRadar(questions, gradingResults) {
    const dimensions = {
    ...
    // 计算百分比得分
    const calcPercent = (dim) => dim.max > 0 ? Math.min(100, Math.round((dim.score / dim.max) * 100)) : 50;
    return {
...
// AI个性化报告生成
async function generateAIPersonalizedReport(apiKey, testInfo) {
    try {
...
// 从缓存获取题目
async function getQuestionsFromCache(db, subject, grade, questionType, difficulty, count) {
    try {
        // 查询已审核的高质量缓存题目
    ...
    `).bind(subject, grade, questionType, difficulty, count).all();
        ...
        return result.results.map((q) => ({
            ...
            scoringPoints: q.scoring_points ? JSON.parse(q.scoring_points) : undefined,
            difficulty: q.difficulty,
            difficultyWeight: difficultyWeights[difficulty],
            estimatedTime: estimatedTimes[questionType][difficulty],
            knowledgePoints: q.knowledge_points ? JSON.parse(q.knowledge_points) : [],
            topic: q.topic,
...
// 保存题目到缓存
async function saveQuestionsToCache(db, subject, grade, questions) {
    try {
      ...
      `).bind(subject, grade, q.questionType, q.questionText).first();
        ...
        `).bind(cacheId, grade, subject, q.topic || null, q.questionType, q.difficulty, q.questionText, q.options ? JSON.stringify(q.options) : null, q.correctAnswer, q.scoringPoints ? JSON.stringify(q.scoringPoints) : null, JSON.stringify(q.knowledgePoints), q.estimatedTime, now, now).run();
            }
        }
    }
    catch (error) {
...
// 更新缓存题目使用统计
async function updateCacheUsageStats(db, questionIds) {
    try {
        const now = new Date().toISOString();
...
// 水平测试题目生成函数（带缓存支持）
async function generateLevelTestQuestionsWithCache(db, subject, grade, distribution, apiKey) {
    const allQuestions = [];
    const questionsToGenerate = [];
    // 难度分配
    const difficultyDistribution = (count) => ({
    ...
    for (const [type, count] of Object.entries(distribution)) {
        ...
        for (const [diff, diffCount] of Object.entries(diffDist)) {
            if (diffCount > 0) {
...
// 水平测试题目生成函数
async function generateLevelTestQuestions(subject, grade, distribution, apiKey) {
...
// 备用题目生成
function generateFallbackLevelTestQuestions(subject, grade, distribution) {
    const questions = [];
    // 根据科目生成基础题目
...
// AI评分主观题
async function gradeSubjectiveAnswer(questionText, correctAnswer, userAnswer, scoringPoints, maxScore, apiKey) {
...
// 数字生成器 - 避免常见数字
function generateDiverseNumbers(difficulty, count) {
    const ranges = {
...
// 选择随机场景
function selectRandomScenario(subject) {
    const scenarios = SCENARIO_LIBRARY[subject] || SCENARIO_LIBRARY.math;
    return scenarios[Math.floor(Math.random() * scenarios.length)];
}
// 生成多样性提示词
function generateDiversityInstructions(recentTopics) {
    const instructions = [
        '确保每道题目的情境和背景都完全不同',
...
// 各科目题目类型指导
const SUBJECT_QUESTION_GUIDANCE = {
...
async function generateQuizQuestions(config, apiKey) {
...
// DeepSeek API 调用
async function analyzeWithDeepSeek(studentInfo, apiKey) {
    if (!apiKey) {
...
// 大学申请分析
async function analyzeUniversityApplication(input, bestFive, apiKey) {
    if (!apiKey) {
...
// 构建大学申请分析提示词
function buildUniversityAnalysisPrompt(input, bestFive) {
    ...
    async fetch(request, env) {
        ...
        try {
            ...
            if (path === '/api/auth/register' && request.method === 'POST') {
                ...
                const now = new Date().toISOString();
                await env.DB.prepare('INSERT INTO users (id, name, email, password_hash, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)').bind(userId, name.trim(), email.toLowerCase().trim(), passwordHash, now, now).run();
            ...
            if (path === '/api/auth/login' && request.method === 'POST') {
                ...
                const user = await env.DB.prepare('SELECT id, name, email, password_hash, avatar, created_at FROM users WHERE email = ?').bind(email.toLowerCase().trim()).first();
                if (!user) {
            ...
            if (path.startsWith('/api/analysis/result/') && request.method === 'GET') {
                ...
                const record = await env.DB.prepare('SELECT id, student_info, result, created_at FROM analysis_records WHERE id = ?').bind(id).first();
                if (!record) {
            ...
            if (path.startsWith('/api/analysis/history/') && request.method === 'DELETE') {
                ...
                // 验证记录是否属于当前用户
                const record = await env.DB.prepare('SELECT id FROM analysis_records WHERE id = ? AND user_id = ?').bind(recordId, tokenData.userId).first();
                if (!record) {
            ...
            if (path === '/api/analysis/feasibility' && request.method === 'POST') {
                ...
                for (const [subject, score] of Object.entries(scores)) {
                    if (score < 50)
                        weakSubjects.push(SUBJECT_NAMES[subject] || subject);
                    else if (score >= 75)
                        strongSubjects.push(SUBJECT_NAMES[subject] || subject);
                }
                ...
                if (strongSubjects.length >= 2) {
                    positiveFactors.push(`多个科目表现突出（${strongSubjects.join('、')}）`);
                }
                ...
                // 生成综合评估描述
                const gradeMap = { 'S1': '中一', 'S2': '中二', 'S3': '中三', 'S4': '中四', 'S5': '中五', 'S6': '中六' };
                ...
                if (weakSubjs.length > 0)
                    recommendations.push(`加强${weakSubjs.map(s => s.subject).join('、')}的训练，确保达到目标学校期望水平`);
           ...
           VALUES (?, ?, 'feasibility', ?, ?, ?)`).bind(recordId, userId, JSON.stringify(body), JSON.stringify({
                    feasibilityLevel,
            ...
            if (path === '/api/student/residence' && request.method === 'POST') {
                ...
                await env.DB.prepare(`INSERT INTO student_residence (id, user_id, district, address, max_commute_time, transport_preference, cross_district, created_at, updated_at) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(id, tokenData.userId, body.district, body.address || null, body.maxCommuteTime || 60, body.transportPreference || 'public', body.crossDistrict ? 1 : 0, now, now).run();
            ...
            if (path === '/api/student/residence' && request.method === 'GET') {
                ...
                const residence = await env.DB.prepare('SELECT * FROM student_residence WHERE user_id = ?').bind(tokenData.userId).first();
            ...
            if (path === '/api/student/preferences' && request.method === 'POST') {
                ...
                await env.DB.prepare(`INSERT INTO school_preferences (id, user_id, school_type, religion_preference, extracurricular_importance, created_at) 
           VALUES (?, ?, ?, ?, ?, ?)`).bind(id, tokenData.userId, body.schoolType || 'coed', body.religionPreference || null, body.extracurricularImportance || 3, now).run();
            ...
            if (path === '/api/schools/recommend' && request.method === 'POST') {
                ...
                // 计算匹配分数
                const recommendations = (schools.results || []).map((school) => {
           ...
           VALUES (?, ?, 'university', ?, ?, ?)`).bind(recordId, userId, JSON.stringify({ ...body, bestFive, bestSix }), JSON.stringify(universityAnalysisResult), now).run();
            ...
            if (path === '/api/trends/employment' && request.method === 'GET') {
                const trends = await env.DB.prepare('SELECT * FROM employment_trends ORDER BY growth_rate DESC').all();
            ...
            if (path === '/api/inquiry/submit' && request.method === 'POST') {
                ...
                const id = crypto.randomUUID();
                const now = new Date().toISOString();
                await env.DB.prepare('INSERT INTO customer_inquiries (id, name, phone, email, message, created_at) VALUES (?, ?, ?, ?, ?, ?)').bind(id, body.name, body.phone || null, body.email || null, body.message, now).run();
            ...
            if (path === '/api/admin/inquiries' && request.method === 'GET') {
                ...
                const inquiries = await env.DB.prepare('SELECT * FROM customer_inquiries ORDER BY created_at DESC').all();
            ...
            if (path.startsWith('/api/admin/inquiry/') && request.method === 'PUT') {
                ...
                const body = await request.json();
                const now = new Date().toISOString();
                await env.DB.prepare('UPDATE customer_inquiries SET status = ?, notes = ?, updated_at = ? WHERE id = ?').bind(body.status || 'pending', body.notes || null, now, inquiryId).run();
            ...
            if (path === '/api/admin/stats' && request.method === 'GET') {
                ...
                const todayUsersCount = await env.DB.prepare('SELECT COUNT(*) as count FROM users WHERE DATE(created_at) = ?').bind(today).first();
                // 获取分析记录总数
                const analysisCount = await env.DB.prepare('SELECT COUNT(*) as count FROM analysis_records').first();
                // 获取今日分析数
                const todayAnalysisCount = await env.DB.prepare('SELECT COUNT(*) as count FROM analysis_records WHERE DATE(created_at) = ?').bind(today).first();
                // 获取最近注册的用户列表（最近10个）
                const recentUsers = await env.DB.prepare('SELECT id, name, email, created_at FROM users ORDER BY created_at DESC LIMIT 10').all();
                return jsonResponse({
                    stats: {
          ...
          `).bind(...limitedIds).all();
            ...
            `).bind(searchPattern, searchPattern, limit).all();
                    ...
                    // 隐藏邮箱中间部分保护隐私
                    const maskedUsers = (users.results || []).map((user) => {
          ...
          `;
                    ...
                    const users = await Promise.all((usersResult.results || []).map(async (user) => {
                        ...
                        try {
                            // 尝试获取积分
                            const pointsResult = await env.DB.prepare('SELECT COALESCE(balance, 0) as balance FROM user_points_account WHERE user_id = ?').bind(user.id).first();
                            points = pointsResult?.balance || 0;
                        }
                        catch { /* 表可能不存在 */ }
                        try {
                            // 尝试获取分析次数
                            const analysisResult = await env.DB.prepare('SELECT COUNT(*) as count FROM analysis_records WHERE user_id = ?').bind(user.id).first();
                            analysis_count = analysisResult?.count || 0;
                        }
                        catch { /* 表可能不存在 */ }
                        try {
                            // 尝试获取测试次数
                            const testResult = await env.DB.prepare('SELECT COUNT(*) as count FROM level_tests WHERE user_id = ?').bind(user.id).first();
                            test_count = testResult?.count || 0;
                        }
                        catch { /* 表可能不存在 */ }
                        try {
                            // 尝试获取发帖数
                            const postResult = await env.DB.prepare('SELECT COUNT(*) as count FROM posts WHERE user_id = ? AND is_deleted = 0').bind(user.id).first();
                ...
                try {
                    ...
                    const safeDelete = async (query, ...params) => {
                        try {
                            ...
                            // 忽略表不存在的错误
                            console.log(`Safe delete skipped: ${e.message}`);
                        }
                    };
                    // 1. 删除用户的帖子和评论
            ...
            if (path === '/api/admin/level-test/stats' && request.method === 'GET') {
                ...
                try {
                    // 测试总数
                    const totalTests = await env.DB.prepare('SELECT COUNT(*) as count FROM level_tests').first();
                    // 已完成测试数
                    const completedTests = await env.DB.prepare('SELECT COUNT(*) as count FROM level_tests WHERE status = ?').bind('graded').first();
                    // 今日测试数
                    const today = new Date().toISOString().split('T')[0];
                    const todayTests = await env.DB.prepare('SELECT COUNT(*) as count FROM level_tests WHERE DATE(created_at) = ?').bind(today).first();
                    // 平均分数
                    const avgScore = await env.DB.prepare('SELECT AVG(score) as avg FROM level_tests WHERE score IS NOT NULL').first();
                    // 各科目测试分布
                    const subjectDistribution = await env.DB.prepare(`SELECT subject, COUNT(*) as count, AVG(score) as avg_score 
             FROM level_tests 
             GROUP BY subject 
             ORDER BY count DESC`).all();
                    // 各年级测试分布
                    const gradeDistribution = await env.DB.prepare(`SELECT grade, COUNT(*) as count, AVG(score) as avg_score 
             FROM level_tests 
             GROUP BY grade`).all();
                    // 等级分布
                    const levelDistribution = await env.DB.prepare(`SELECT overall_level, COUNT(*) as count 
             FROM level_tests 
             WHERE overall_level IS NOT NULL 
             GROUP BY overall_level`).all();
                    // 缓存题目统计
                    const cachedQuestions = await env.DB.prepare('SELECT COUNT(*) as count FROM question_cache').first();
                    // 待审核题目数
                    const pendingReviews = await env.DB.prepare('SELECT COUNT(*) as count FROM question_review_queue WHERE status = ?').bind('pending').first();
                    return jsonResponse({
            ...
            if (path === '/api/admin/level-test/review-queue' && request.method === 'GET') {
                ...
                try {
                    ...
                    const reviews = await env.DB.prepare(`SELECT qr.*, qc.question_data, qc.grade, qc.subject, qc.difficulty, qc.question_type
             FROM question_review_queue qr
             LEFT JOIN question_cache qc ON qr.question_id = qc.id
             WHERE qr.status = ?
             ORDER BY qr.created_at DESC
             LIMIT ? OFFSET ?`).bind(status, limit, offset).all();
                    const total = await env.DB.prepare('SELECT COUNT(*) as count FROM question_review_queue WHERE status = ?').bind(status).first();
                    return jsonResponse({
            ...
            if (path.startsWith('/api/admin/level-test/question/') && path.endsWith('/review') && request.method === 'PUT') {
                ...
                try {
                    ...
                    const now = new Date().toISOString();
                    // 更新审核状态
                    await env.DB.prepare(`UPDATE question_review_queue 
             SET status = ?, review_comments = ?, reviewed_at = ?, updated_at = ?
             WHERE question_id = ?`).bind(body.status, body.comments || '', now, now, questionId).run();
                    // 如果题目被修改，更新缓存中的题目数据
                    if (body.status === 'modified' && body.modifiedData) {
                        await env.DB.prepare(`UPDATE question_cache SET question_data = ?, updated_at = ? WHERE id = ?`).bind(body.modifiedData, now, questionId).run();
                    }
                    // 如果题目被拒绝，从缓存中删除
                    if (body.status === 'rejected') {
                        await env.DB.prepare('DELETE FROM question_cache WHERE id = ?').bind(questionId).run();
                    }
          ...
          `;
            ...
            if (path.match(/^\/api\/admin\/level-test\/\w+$/) && request.method === 'GET') {
                ...
                try {
                    ...
                    const test = await env.DB.prepare(`SELECT lt.*, u.name as user_name, u.email as user_email
             FROM level_tests lt
             LEFT JOIN users u ON lt.user_id = u.id
             WHERE lt.id = ?`).bind(testId).first();
                    if (!test) {
                        return errorResponse('测试不存在', 404, origin);
                    }
                    const questions = await env.DB.prepare('SELECT * FROM test_questions WHERE test_id = ? ORDER BY id').bind(testId).all();
                    const report = await env.DB.prepare('SELECT * FROM test_reports WHERE test_id = ?').bind(testId).first();
                    return jsonResponse({
                        test,
            ...
            if (path === '/api/admin/level-test/cache/add' && request.method === 'POST') {
                ...
                try {
                    ...
                    for (const q of body.questions) {
                        try {
                            const id = crypto.randomUUID();
                            await env.DB.prepare(`INSERT INTO question_cache (id, grade, subject, question_type, difficulty, question_data, created_at, updated_at)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).bind(id, q.grade, q.subject, q.question_type, q.difficulty, q.question_data, now, now).run();
                            addedCount++;
                        }
                        catch (e) {
            ...
            if (path === '/api/admin/level-test/cache/cleanup' && request.method === 'POST') {
                ...
                try {
                    ...
                    // 删除使用次数足够但正确率过低的题目
                    const result = await env.DB.prepare(`DELETE FROM question_cache 
             WHERE usage_count >= ? AND avg_score_rate < ?`).bind(minUsageCount, minScoreRate).run();
                    return jsonResponse({
                        success: true,
            ...
            if (path === '/api/quiz/start' && request.method === 'POST') {
                ...
                if (authHeader?.startsWith('Bearer ')) {
                    ...
                    if (tokenData) {
                        try {
                            await env.DB.prepare(`INSERT INTO quiz_sessions (id, user_id, config, status, questions, start_time) 
                 VALUES (?, ?, ?, 'active', ?, ?)`).bind(sessionId, tokenData.userId, JSON.stringify(body), JSON.stringify(questions), new Date().toISOString()).run();
                        }
                        catch (e) {
            ...
            if (path === '/api/quiz/save' && request.method === 'POST') {
                ...
                try {
                    const now = new Date().toISOString();
                    // 检查是否已存在该session
                    const existingSession = await env.DB.prepare('SELECT id FROM quiz_sessions WHERE id = ?').bind(body.sessionId).first();
              ...
              SET status = 'completed', 
                  ...
                  questions = ?
              WHERE id = ?
            `).bind(now, body.score, body.accuracy / 100, body.timeSpent, JSON.stringify(body.questions), body.sessionId).run();
                    }
                    else {
                        // 插入新记录
                        await env.DB.prepare(`
            ...
            `).bind(body.sessionId, tokenData.userId, JSON.stringify(body.config), JSON.stringify(body.questions), now, now, body.score, body.accuracy / 100, body.timeSpent, now).run();
                    ...
                    if (existingStats) {
                        const totalSessions = (existingStats.total_sessions || 0) + 1;
                        const totalQuestions = (existingStats.total_questions || 0) + body.config.questionCount;
                        const correctAnswers = (existingStats.correct_answers || 0) + body.score;
                        await env.DB.prepare(`
              ...
              SET total_sessions = ?, 
                  ...
                  updated_at = ?
              WHERE user_id = ?
            `).bind(totalSessions, totalQuestions, correctAnswers, correctAnswers / totalQuestions, now, now, tokenData.userId).run();
                    }
                    else {
                        const id = crypto.randomUUID();
                        await env.DB.prepare(`
              ...
              VALUES (?, ?, 1, ?, ?, ?, ?, ?, ?)
            `).bind(id, tokenData.userId, body.config.questionCount, body.score, body.accuracy / 100, now, now, now).run();
                    }
                    return jsonResponse({
                        success: true,
                        message: '刷题记录已保存',
          ...
          `).bind(userId, monthStart.toISOString()).all();
            ...
            if (path === '/api/leaderboard' && request.method === 'GET') {
                ...
                try {
                    ...
                    const bindParams = [periodStart.toISOString()];
                    // 科目筛选
          ...
          `;
                    const results = await env.DB.prepare(query)
                        ...
                        .all();
                    // 计算综合得分并生成排名
                    const rankings = (results.results || []).map((row, index) => {
                        const accuracy = row.avg_accuracy || 0;
                        const avgTime = row.avg_time || 60;
                        const avgTimePerQuestion = row.avg_time_per_question || 30;
          ...
          WHERE user_id = ? AND status = 'completed'
        `;
                const stats = await env.DB.prepare(statsQuery).bind(tokenData.userId).first();
        ...
        `;
                const last7Days = await env.DB.prepare(last7DaysQuery).bind(tokenData.userId).first();
        ...
        `;
                ...
                const recentScores = (recentResults.results || []).map((r) => Math.round((r.accuracy || 0) * 100));
                const userStats = {
                    totalSessions: stats.total_sessions,
                    totalQuestions: stats.total_questions || 0,
                    correctAnswers: Math.round((stats.avg_accuracy || 0) * (stats.total_questions || 0)),
                    totalTimeSpent: 0,
                    averageAccuracy: Math.round((stats.avg_accuracy || 0) * 100),
                    averageTimePerQuestion: Math.round((stats.avg_time || 0) * 10) / 10,
                    currentStreak: 0,
                    longestStreak: 0,
            ...
            if (path === '/api/leaderboard/submit' && request.method === 'POST') {
                ...
                if (existingStats) {
                    // 更新现有统计
                    const totalSessions = (existingStats.total_sessions || 0) + 1;
                    const totalQuestions = (existingStats.total_questions || 0) + body.questionCount;
                    const correctAnswers = (existingStats.correct_answers || 0) + Math.round(body.accuracy * body.questionCount);
                    await env.DB.prepare(`
            UPDATE user_ranking_stats 
            SET total_sessions = ?, 
                total_questions = ?, 
          ...
          `).bind(totalSessions, totalQuestions, correctAnswers, correctAnswers / totalQuestions, now, now, tokenData.userId).run();
            ...
            (id, user_id, total_sessions, total_questions, correct_answers, average_accuracy, last_activity_at, created_at, updated_at)
            VALUES (?, ?, 1, ?, ?, ?, ?, ?, ?)
          `).bind(id, tokenData.userId, body.questionCount, Math.round(body.accuracy * body.questionCount), body.accuracy, now, now, now).run();
                }
            ...
            if (path === '/api/user/profile' && request.method === 'PUT') {
                ...
                try {
                    const body = await request.json();
                    if (body.name) {
                        await env.DB.prepare('UPDATE users SET name = ?, updated_at = ? WHERE id = ?').bind(body.name, new Date().toISOString(), tokenData.userId).run();
                    }
            ...
            if (path === '/api/user/avatar' && request.method === 'POST') {
                ...
                try {
                    const body = await request.json();
                    // 保存头像到数据库（base64格式）
                    await env.DB.prepare('UPDATE users SET avatar = ?, updated_at = ? WHERE id = ?').bind(body.avatar, new Date().toISOString(), tokenData.userId).run();
            ...
            if (path === '/api/user/password' && request.method === 'PUT') {
                ...
                try {
                    const body = await request.json();
                    // 获取用户当前密码
                    const user = await env.DB.prepare('SELECT password_hash FROM users WHERE id = ?').bind(tokenData.userId).first();
                    if (!user) {
                        return errorResponse('用户不存在', 404, origin);
                    }
                    ...
                    // 更新密码
                    await env.DB.prepare('UPDATE users SET password_hash = ?, updated_at = ? WHERE id = ?').bind(body.newPassword, new Date().toISOString(), tokenData.userId).run();
            ...
            if (path === '/api/level-test/generate' && request.method === 'POST') {
                ...
                try {
                    ...
                    const now = new Date().toISOString();
                    // 生成题目（优先使用缓存，不足时调用AI）
                    const questions = await generateLevelTestQuestionsWithCache(env.DB, body.subject, body.grade, config.distribution, env.DEEPSEEK_API_KEY);
                    // 保存测试到数据库
                    await env.DB.prepare(`
          ...
          `).bind(testId, tokenData.userId, body.grade, body.subject, body.testType, config.timeLimit, now, now).run();
                    // 保存题目到数据库
            ...
            `).bind(q.id, testId, i, q.questionText, q.questionType, q.options ? JSON.stringify(q.options) : null, q.correctAnswer, q.scoringPoints ? JSON.stringify(q.scoringPoints) : null, q.maxScore, q.difficulty, q.difficultyWeight, q.estimatedTime, JSON.stringify(q.knowledgePoints), q.topic || null, now).run();
                    }
                    return jsonResponse({
                        success: true,
                        testId,
            ...
            if (path.startsWith('/api/level-test/') && path.endsWith('/questions') && request.method === 'GET') {
                ...
                try {
                    // 获取测试信息
                    const test = await env.DB.prepare('SELECT * FROM level_tests WHERE id = ? AND user_id = ?').bind(testId, tokenData.userId).first();
                    if (!test) {
                        return errorResponse('测试不存在', 404, origin);
                    }
                    // 获取题目（不返回正确答案）
          ...
          `).bind(testId).all();
                    ...
                    if (test.status === 'pending') {
                        await env.DB.prepare('UPDATE level_tests SET status = ?, started_at = ?, updated_at = ? WHERE id = ?').bind('in_progress', new Date().toISOString(), new Date().toISOString(), testId).run();
                    }
                    return jsonResponse({
                        testId,
                        grade: test.grade,
            ...
            `).bind(ans.answer, ans.timeSpent || null, now, ans.questionId, testId).run();
                    }
                    // 更新标记状态
                    if (body.markedQuestions && body.markedQuestions.length > 0) {
                        // 先清除所有标记
                        await env.DB.prepare('UPDATE test_questions SET is_marked = 0 WHERE test_id = ?').bind(testId).run();
                        // 设置新标记
                        for (const idx of body.markedQuestions) {
                            await env.DB.prepare('UPDATE test_questions SET is_marked = 1 WHERE test_id = ? AND question_index = ?').bind(testId, idx).run();
                        }
                    }
                    // 保存自动保存记录
          ...
          `).bind(autosaveId, testId, tokenData.userId, JSON.stringify(body.answers), body.currentIndex, body.timeRemaining, JSON.stringify(body.markedQuestions || []), now).run();
                    // 更新测试进度
                    await env.DB.prepare(`
            UPDATE level_tests 
          ...
          `).bind(body.currentIndex, body.answers.length, now, testId).run();
            ...
            if (path.startsWith('/api/level-test/') && path.endsWith('/submit') && request.method === 'POST') {
                ...
                try {
                    ...
                    // 获取所有题目
                    const questionsResult = await env.DB.prepare('SELECT * FROM test_questions WHERE test_id = ? ORDER BY question_index').bind(testId).all();
                    ...
                    for (const q of questions) {
                        ...
                        if (questionType === 'choice') {
                            ...
                            // 短答题和论述题使用AI评分或关键词匹配
                            const result = await gradeSubjectiveAnswer(q.question_text, correctAnswer, userAnswer, q.scoring_points ? JSON.parse(q.scoring_points) : [], maxScore, env.DEEPSEEK_API_KEY);
                            score = result.score;
            ...
            WHERE id = ?
          `).bind(totalScore, finalScore, level, body.totalTimeSpent, now, now, now, testId).run();
                    // 生成报告
                    ...
                    for (let i = 0; i < questions.length; i++) {
                        ...
                        const kps = q.knowledge_points ? JSON.parse(q.knowledge_points) : [];
                        for (const kp of kps) {
                    ...
                    // 生成AI个性化报告（异步但等待结果）
                    const aiReport = await generateAIPersonalizedReport(env.DEEPSEEK_API_KEY, {
                        subject: test.subject,
                        grade: test.grade,
                        score: finalScore,
                        level,
          ...
          `).bind(testId).all();
                    ...
                    return jsonResponse({
                        ...
                        abilityRadar: JSON.parse(report.ability_radar),
                        strengthPoints: JSON.parse(report.strength_points),
                        weaknessPoints: JSON.parse(report.weakness_points),
          ...
          `;
                    ...
                    const tests = result.results.map((t) => ({
                        ...
                        level: t.level,
                        levelDescription: levelToDescription(t.level),
            ...
            WHERE user_id = ? AND status = 'graded'
          `;
                    const countParams = [tokenData.userId];
                    if (subject) {
                        countQuery += ' AND subject = ?';
                        countParams.push(subject);
                    }
