 ...
 * 构建用户 Prompt
 */
function buildUserPrompt(ruleScore, student, targetSchool) {
 ...
 * 调用 DeepSeek API
 */
async function callDeepSeekAPI(systemPrompt, userPrompt, apiKey, endpoint = 'https://api.deepseek.com/v1/chat/completions', model = 'deepseek-chat') {
 ...
 * 解析并验证 AI 输出
 */
function parseAndValidateOutput(content, expectedLevel) {
    try {
        // 清理可能的 markdown 包装
 ...
 * @returns AI 分析输出
 */
export async function generateAIExplanation(ruleScore, student, targetSchool, apiKey, maxRetries = 2) {
 ...
 * 生成本地备用输出（当 AI 调用失败时）
 */
function generateFallbackOutput(ruleScore) {
