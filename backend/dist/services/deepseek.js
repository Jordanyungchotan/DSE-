 ...
 * 构建分析提示词
 */
const buildAnalysisPrompt = (studentInfo) => {
    // 格式化科目成绩
 ...
 * 调用DeepSeek API进行分析
 */
export const analyzeWithDeepSeek = async (studentInfo) => {
    // 检查API密钥
 ...
 */
const generateMockResult = (studentInfo) => {
    // 计算基础可行性评分
    let baseScore = 70;
    // 根据年级调整
    if (studentInfo.grade === 'form6')
        baseScore -= 10;
    ...
    // 生成学校评估
    const schoolAssessments = studentInfo.targetSchools.map((school, index) => ({
        schoolName: school,
