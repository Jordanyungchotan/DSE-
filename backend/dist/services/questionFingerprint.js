...
export class QuestionFingerprintGenerator {
     ...
     * 提取结构特征
     */
    extractStructuralFeatures(text, questionType) {
        // 句子数量（按句号、问号分割）
     ...
     * 计算复杂度
     */
    calculateComplexity(text, hasFormula, sentenceCount) {
        let score = 0;
        // 基于长度
        if (text.length > 200)
            score += 2;
    ...
    extractNumericalPattern(text) {
        ...
        if (maxMagnitude > 10000)
            magnitudeRange = 'large';
        else if (maxMagnitude > 100)
            magnitudeRange = 'medium';
        // 提取单位
     ...
     * 提取概念签名
     */
    extractConceptualSignature(question) {
    ...
    calculateSimilarity(fp1, fp2) {
        ...
        const overallSim = semanticSim * weights.semantic +
     ...
     * 计算语义相似度
     */
    calculateSemanticSimilarity(hash1, hash2) {
        if (hash1 === hash2)
            return 1.0;
        // 计算哈希的汉明距离
        let diff = 0;
     ...
     * 计算结构相似度
     */
    calculateStructuralSimilarity(sf1, sf2) {
        let score = 0;
        let total = 0;
        // 题型匹配
        if (sf1.questionType === sf2.questionType)
            score += 2;
        total += 2;
        // 句子数量接近度
        ...
        total += 1;
        // 特征匹配
        if (sf1.hasFormula === sf2.hasFormula)
            score += 1;
        if (sf1.hasGraph === sf2.hasGraph)
            score += 0.5;
        if (sf1.hasTable === sf2.hasTable)
            score += 0.5;
        total += 2;
        // 问题模式匹配
        if (sf1.questionPattern === sf2.questionPattern)
            score += 1.5;
        total += 1.5;
        // 复杂度接近度
     ...
     * 计算数值相似度
     */
    calculateNumericalSimilarity(np1, np2) {
        // 如果数值哈希完全相同
        if (np1.numericHash === np2.numericHash)
            return 1.0;
        let score = 0;
        let total = 0;
        // 数字数量相似
        ...
        total += 1;
        // 特征匹配
        if (np1.hasDecimals === np2.hasDecimals)
            score += 0.5;
        if (np1.hasFractions === np2.hasFractions)
            score += 0.5;
        if (np1.hasNegatives === np2.hasNegatives)
            score += 0.5;
        total += 1.5;
        // 数量级范围匹配
        if (np1.magnitudeRange === np2.magnitudeRange)
            score += 1;
        total += 1;
        // 单位重叠度
     ...
     * 计算数字重叠度
     */
    calculateNumberOverlap(nums1, nums2) {
     ...
     * 计算概念相似度
     */
    calculateConceptualSimilarity(cs1, cs2) {
        let score = 0;
        let total = 0;
        // 科目匹配
        if (cs1.subject === cs2.subject)
            score += 2;
        total += 2;
        // 知识点重叠
        ...
        total += 1;
        // 难度匹配
        if (cs1.difficulty === cs2.difficulty)
            score += 1;
        total += 1;
        // 认知层次匹配
        if (cs1.cognitiveLevel === cs2.cognitiveLevel)
            score += 1;
        total += 1;
        return score / total;
    }
...
export class QuestionDeduplicator {
     ...
     * 过滤重复题目
     */
    async filterDuplicates(newQuestions, existingFingerprints) {
        const unique = [];
        const duplicates = [];
        const variants = [];
        // 生成新题目的指纹
     ...
     * 计算题目集合的多样性分数
     */
    async calculateDiversityScore(questions) {
        ...
        const overallScore = (structuralDiversity * 0.25 +
    ...
    calculateStructuralDiversity(fps) {
        // 统计不同题型的数量
    ...
    calculateNumericalDiversity(fps) {
        // 检查数值模式的多样性
    ...
    calculateConceptualDiversity(fps) {
        // 检查概念的多样性
        const allTopics = new Set();
        const allSkills = new Set();
        const cogLevels = new Set();
        for (const fp of fps) {
...
//# sourceMappingURL=questionFingerprint.js.map