 ...
 * @version v2
 */
import { DEFAULT_CAPABILITY_ANALYSES, DEFAULT_TRANSITION_PLAN, DEFAULT_SUMMARY_ADVANTAGES, DEFAULT_SUMMARY_RISKS, DEFAULT_SCHOOL_ASSESSMENT_FIELDS, getFeasibilityLevel, getRecommendationType, getRiskLevel, mapSelfAssessmentLevel, } from '../templates/transferDefaults';
 ...
 */
export function executeTransferAnalysisV2(input) {
...
目标学校评估（不可修改）：
${ruleResult.schoolAssessments.map(s => `- ${s.schoolName}：匹配度 ${s.matchScore}%，风险 ${s.riskLevel}，建议 ${s.recommendation}`).join('\n')}
 ...
 */
export async function callAIEnhancement(apiKey, input, ruleResult) {
    if (!apiKey) {
...
function validateAndSanitizeAIResult(aiResult) {
    ...
    if (aiResult.capabilityAnalyses && Array.isArray(aiResult.capabilityAnalyses)) {
        ...
        result.capabilityAnalyses = aiResult.capabilityAnalyses
            .filter(ca => validDimensions.includes(ca.dimension) &&
    ...
    if (aiResult.transitionPlan) {
        ...
        if (Array.isArray(plan.shortTerm) && plan.shortTerm.length >= 2 &&
 ...
 */
export function mergeAIEnhancement(ruleResult, aiResult) {
    // 如果 AI 增强失败，直接返回规则结果
    if (!aiResult) {
        return ruleResult;
    }
...
// 辅助函数
// ============================================
function buildKeyAdvantages(ruleAnalysis, input) {
    const advantages = [];
    // 从规则分析中提取优势
...
function buildKeyRisks(ruleAnalysis, input) {
    const risks = [];
    // 从规则分析中提取风险
 ...
 * - 纯规则生成
 */
function buildDecisionBasis(ruleAnalysis, input, feasibilityScore) {
    const basis = [];
    // 1. 科目状态分布
    const statusCounts = {
...
function buildCapabilityAnalyses(input, feasibilityScore) {
    // 基于用户自评和可行性分数调整能力分析
...
function buildSchoolAssessments(targetSchools, feasibilityScore, targetGrade) {
 ...
 * - 风险判断基于推断而非事实
 */
export function detectDataGaps(input, schoolAssessments) {
    const triggerReasons = [];
    const schoolsWithGaps = [];
 ...
 * 构建 AI 外部数据核验 Prompt
 */
function buildExternalVerificationPrompt(input, schoolAssessments, triggerReasons) {
...
3. 解释为什么某些数据缺失
4. 给出针对性的准备建议`;
    const schoolsInfo = schoolAssessments.map(s => `- ${s.schoolName}（当前评估：匹配度 ${s.matchScore}%，风险 ${s.riskLevel}）`).join('\n');
 ...
 */
export async function callExternalDataVerification(apiKey, input, schoolAssessments, triggerReasons) {
 ...
 * 验证 AI 结果并构建核验结果
 */
function validateAndBuildVerification(aiResult, triggerReasons, latency) {
    // 验证 dataAvailability
    const validAvailability = ['充分', '有限', '极少', '几乎没有'];
    const dataAvailability = validAvailability.includes(aiResult.dataAvailability)
        ? aiResult.dataAvailability
        : '有限';
    // 验证 impactOnAssessment
    const validImpact = ['不影响', '轻微影响', '明显影响'];
    const impactOnAssessment = validImpact.includes(aiResult.impactOnAssessment)
        ? aiResult.impactOnAssessment
        : '轻微影响';
    // 构建 publicFindings
    const publicFindings = Array.isArray(aiResult.publicFindings)
        ? aiResult.publicFindings.slice(0, 5).map(s => truncateString(String(s), 200))
    ...
    const recommendedActions = Array.isArray(aiResult.recommendedActions)
        ? aiResult.recommendedActions.slice(0, 5).map(s => truncateString(String(s), 150))
 ...
 * 创建默认核验结果（当 AI 调用失败时）
 */
function createDefaultVerification(triggerReasons) {
    return {
        triggered: true,
        triggerReasons,
        dataAvailability: '有限',
...
function getDefaultPublicFindings() {
    return [
 ...
 * 为学校评估添加数据缺口解释和融合结论
 */
export function enrichSchoolAssessmentsWithVerification(schoolAssessments, aiSchoolFindings, verification) {
    return schoolAssessments.map(school => {
        // 查找 AI 对该学校的特定分析
        const aiFindings = aiSchoolFindings?.find(f => f.schoolName === school.schoolName);
        // 构建数据缺口解释
        const dataGapExplanations = [];
 ...
 */
export function enrichSummaryWithVerification(summary, verification) {
    // 构建全局数据缺口解释
    const dataGapExplanations = [{
            gapType: '整体插班市场数据',
    ...
    // 构建综合现实结论
    const integratedRealityConclusion = `系统规则评估整体可行性为「${summary.overallLevel}」（${summary.feasibilityScore}分）。` +
 ...
 */
export async function executeTransferAnalysisV2WithVerification(input, apiKey) {
    ...
    // Step 4: 丰富学校评估
    const enrichedSchoolAssessments = enrichSchoolAssessmentsWithVerification(ruleResult.schoolAssessments, undefined, // AI 特定学校分析（如果有的话）
    verification);
    // Step 5: 丰富 Summary
 ...
 */
function isDefaultVerificationTemplate(verification) {
...
//# sourceMappingURL=transferAnalysisV2Service.js.map