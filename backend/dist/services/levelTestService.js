...
export function levelToDescription(level) {
    const descriptions = {
        '5**': '优异 - 表现卓越，掌握高阶思维能力',
        '5*': '优良 - 全面掌握，能灵活应用',
        '5': '良好 - 扎实掌握核心概念',
 ...
 * 计算测试所需题目数量
 */
export function calculateQuestionDistribution(testType) {
    ...
    // 按难度分配每种题型
    const distributeByDifficulty = (count) => ({
 ...
 * 计算加权得分
 */
export function calculateWeightedScore(results, questions) {
    let totalRawScore = 0;
    let totalMaxScore = 0;
 ...
 * 分析能力维度
 */
export function analyzeAbilityDimensions(results, questions) {
    // 根据题目类型和难度映射到能力维度
    const dimensions = {
    ...
    // 计算百分比得分
    const calculatePercent = (dim) => dim.max > 0 ? Math.round((dim.score / dim.max) * 100) : 0;
    return {
 ...
 * 识别优势和薄弱知识点
 */
export function analyzeKnowledgePoints(results, questions) {
 ...
 * 计算等价年级水平
 */
export function calculateGradeEquivalent(score, targetGrade) {
 ...
 * 生成学习建议
 */
export function generateRecommendations(weaknessPoints, subject, grade) {
...
//# sourceMappingURL=levelTestService.js.map