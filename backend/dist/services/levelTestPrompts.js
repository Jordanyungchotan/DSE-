...
请使用简体中文回答（除非是英文科目）。`;
// ===== 生成选择题的提示词 =====
export function getChoiceQuestionPrompt(subject, grade, difficulty, count, topics) {
...
// ===== 生成短答题的提示词 =====
export function getShortAnswerPrompt(subject, grade, difficulty, count, topics) {
    const topicList = topics || SUBJECT_TOPICS[subject]?.[grade] || [];
    const topicStr = topicList.length > 0 ? topicList.join('、') : '本年级所有主题';
    const difficultyGuide = {
...
// ===== 生成论述题的提示词 =====
export function getLongAnswerPrompt(subject, grade, difficulty, count, topics) {
    const topicList = topics || SUBJECT_TOPICS[subject]?.[grade] || [];
    const topicStr = topicList.length > 0 ? topicList.join('、') : '本年级所有主题';
    const difficultyGuide = {
        easy: '结构化论述，有明确的答题框架',
...
// ===== 生成综合测试的完整提示词 =====
export function getFullTestPrompt(subject, grade, distribution) {
...
请按以下JSON格式返回：
{
  "test_info": {
    ...
    "total_score": ${distribution.choice.easy * 1 + distribution.choice.medium * 1 + distribution.choice.hard * 2 +
...
// ===== AI评分提示词 =====
export function getGradingPrompt(question, userAnswer) {
...
// ===== 报告生成提示词 =====
export function getReportPrompt(testInfo) {
...
// ===== 英文科目专用提示词 =====
export function getEnglishQuestionPrompt(grade, questionType, difficulty, count) {
...
//# sourceMappingURL=levelTestPrompts.js.map