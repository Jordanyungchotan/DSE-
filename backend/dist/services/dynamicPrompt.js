...
export class DynamicPromptGenerator {
    ...
    generatePrompt(config) {
        ...
        // 构建用户提示词
        const userPrompt = this.buildUserPrompt(config, scenarioType, numberRange, antiRepeatInstructions, recommendedTopics);
        return {
            systemPrompt,
            userPrompt,
            metadata: {
     ...
     * 生成多样性指令
     */
    generateDiversityInstructions(config) {
        const instructions = [];
     ...
     * 生成防重复指令
     */
    generateAntiRepeatInstructions(config) {
        const instructions = [
     ...
     * 获取推荐知识点
     */
    getRecommendedTopics(config) {
        if (config.topic) {
            // 如果指定了主题，获取相关主题
            return [config.topic, ...this.knowledgeGraph.recommendTopics(config.subject, config.userWeakAreas, [], 2)];
        }
        // 否则推荐主题
        return this.knowledgeGraph.recommendTopics(config.subject, config.userWeakAreas, [], 3);
    }
    /**
     * 获取多样性级别描述
     */
    getDiversityLevel(creativity) {
     ...
     * 构建系统提示词
     */
    buildSystemPrompt(config, diversityInstructions) {
...
你必须严格按照JSON格式输出，不要添加任何其他内容。`;
     ...
     * 构建用户提示词
     */
    buildUserPrompt(config, scenario, numberRange, antiRepeatInstructions, recommendedTopics) {
        const subjectName = this.getSubjectName(config.subject);
        const gradeName = this.getGradeName(config.grade);
        const difficultyName = this.getDifficultyName(config.difficulty);
        // 随机选择问题结构
...
\`\`\``;
     ...
     * 选择问题结构
     */
    selectQuestionStructures(questionType, count) {
        const structures = [];
     ...
     * 生成数字建议
     */
    generateNumberSuggestions(difficulty, count) {
        const suggestions = [];
        const ranges = NUMBER_RANGES[difficulty] || NUMBER_RANGES.standard;
        for (let i = 0; i < Math.min(count, 5); i++) {
            const sizeKeys = Object.keys(ranges);
...
export class AdvancedPromptStrategy {
    baseGenerator = new DynamicPromptGenerator();
    /**
     * 基于用户表现生成自适应提示词
     */
    generateAdaptivePrompt(config, userPerformance) {
        // 根据正确率调整难度
     ...
     * 生成变体题目提示词
     */
    generateVariationPrompt(baseQuestion, variationType, config) {
