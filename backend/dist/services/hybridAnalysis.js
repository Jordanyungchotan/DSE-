 ...
 * - 强制校验 AI 输出结构，失败则重试
 */
import { calculatePlacementScore, } from './placementScore.js';
import { LEVEL_DESCRIPTIONS, CONVERSION_COPIES, DISCLAIMER, getLevelScoreRange, getRecommendedActions, } from '../config/index.js';
// ============================================================
// 辅助函数
// ============================================================
/** 科目名称映射 */
 ...
 */
function dseGradeToScore(grade) {
...
/** 转换学生输入为评分引擎格式 */
function convertToStudentProfile(input) {
    const scores = {};
 ...
 */
async function callAIWithRetry(input, ruleResult, apiKey, maxRetries = 1) {
 ...
 * 生成备用解释（AI 不可用或校验失败时）
 */
function generateFallbackOutput(ruleResult) {
 ...
 * 4. 合并返回完整结果
 */
export async function analyzeWithHybridEngine(input, apiKey) {
    // ============================================================
    // Step 1: 规则引擎评分
    // ============================================================
    const studentProfile = convertToStudentProfile(input);
    const primarySchool = input.targetSchools[0]
        ? {
 ...
 * 评估多个目标学校
 */
export async function analyzeMultipleSchools(input, apiKey) {
    const results = [];
...
//# sourceMappingURL=hybridAnalysis.js.map