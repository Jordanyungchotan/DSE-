...
export function validateStudentProfile(student) {
    ...
    else if (!VALID_GRADES.includes(s.currentGrade)) {
 ...
 * 校验目标学校
 */
export function validateTargetSchool(school) {
    const errors = [];
    ...
    else if (!VALID_BANDS.includes(s.bandLevel)) {
    ...
    // 性别（可选）
    if (s.gender !== undefined && !['boys', 'girls', 'coed'].includes(s.gender)) {
        errors.push('gender 必须是 boys、girls 或 coed');
    }
    // 类型（可选）
    if (s.type !== undefined && !['government', 'aided', 'dss', 'private'].includes(s.type)) {
 ...
 * 校验分析提交请求
 */
export function validateAnalysisSubmit(body) {
    const errors = [];
...
export function validateFeedback(body) {
    ...
    else if (!['success', 'failure', 'not_tried', 'pending'].includes(b.userOutcome)) {
 ...
 * 校验咨询预约请求
 */
export function validateConsultationBook(body) {
    const errors = [];
    if (!body || typeof body !== 'object') {
        return { valid: false, errors: ['请求体必须是 JSON 对象'] };
    }
