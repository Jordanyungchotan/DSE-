 ...
 * - ❌ session / store 中的临时值
 */
import { getUserLearningRankWithAnalysis, ANTI_CHEAT_CONFIG, } from './learningLeaderboardService.js';
...
function generateMissionId(type, suffix) {
 ...
 */
export async function getDailyMissions(db, userId) {
 ...
 */
function createFallbackResponse(userId, today) {
    return {
        date: today,
        userId,
 ...
 * 内部任务生成逻辑
 */
async function generateDailyMissionsInternal(db, userId, today) {
    const missions = [];
    // 默认值（用户无数据时使用）
    let quizCountRank = null;
    let accuracyRank = null;
    ...
    try {
        ...
        if (results[0].status === 'fulfilled')
            quizCountRank = results[0].value;
        if (results[1].status === 'fulfilled')
            accuracyRank = results[1].value;
        if (results[2].status === 'fulfilled' && results[2].value)
            todayStats = results[2].value;
        if (results[3].status === 'fulfilled' && results[3].value)
            userStreak = results[3].value;
        if (results[4].status === 'fulfilled')
            mistakeCount = results[4].value;
    }
    catch (error) {
...
// ===== 安全包装函数（捕获所有可能的错误）=====
async function safeGetUserLearningRank(db, userId, metric) {
    try {
...
async function safeTodayQuizStats(db, userId) {
    try {
...
async function safeUserStreak(db, userId) {
    try {
...
async function safeMistakeCount(db, userId) {
    try {
...
async function safeReviewedMistakesToday(db, userId) {
    try {
...
async function safeFindWeakestSubject(db, userId) {
    try {
...
async function safeSubjectProgressToday(db, userId, subjectKey) {
    try {
...
// ===== 数据获取函数 =====
async function getTodayQuizStats(db, userId) {
...
async function getMistakeCount(db, userId) {
...
async function getReviewedMistakesToday(db, userId) {
    // 【关键】从 question_attempts 事实表读取
...
async function findWeakestSubject(db, userId) {
    // 【关键】从 learning_events 事实表读取
    const result = await db.prepare(`
    SELECT 
      subject,
  ...
  `).bind(userId).first();
    if (!result)
        return null;
    // 映射科目名称
    const subjectKey = Object.entries(SUBJECT_NAMES).find(([, name]) => name === result.subject || result.subject.toLowerCase().includes(name.toLowerCase()))?.[0];
    if (subjectKey) {
        return { key: subjectKey, name: SUBJECT_NAMES[subjectKey] };
    }
    return null;
}
async function getSubjectProgressToday(db, userId, subjectKey) {
  ...
  `).bind(userId, subjectName, `%${subjectName}%`).first();
    return result?.count || 0;
}
// ===== 任务创建函数 =====
function calculateDailyQuizTarget(rankInfo, todayStats) {
    // 基础目标：5 道题
    let target = 5;
    // 如果用户有排名且接近前 20，增加目标
...
function createDailyQuizMission(target, current) {
...
function createStreakMission(targetDay, hasQuizzedToday) {
...
function findRankBreakthroughOpportunity(quizRank, accuracyRank) {
    // 检查刷题数量排名
...
function createRankBreakthroughMission(breakthrough) {
...
function createWeaknessMission(weakness) {
...
function createAccuracyBoostMission(currentAccuracy) {
...
function createReviewMistakesMission(target, reviewed) {
...
function createSubjectFocusMission(subjectKey, subjectName, progress) {
 ...
 * 检查并更新任务进度
 */
export async function checkMissionProgress(db, userId, eventType, eventData) {
...
//# sourceMappingURL=dailyMissionService.js.map