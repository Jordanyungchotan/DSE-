...
/** Band 等级难度规则 */
export const BAND_RULES: Record<1 | 2 | 3, BandDifficultyConfig> = {
  1: SchoolDifficultyConfig.band1 as BandDifficultyConfig,
  2: SchoolDifficultyConfig.band2 as BandDifficultyConfig,
  3: SchoolDifficultyConfig.band3 as BandDifficultyConfig,
}

/** 年级难度系数 */
export const GRADE_SENSITIVITY: Record<string, GradeSensitivityConfig> = GradeSensitivity as Record<string, GradeSensitivityConfig>

/** 简化的年级系数（仅数值） */
export const GRADE_FACTOR: Record<string, number> = Object.fromEntries(
  Object.entries(GradeSensitivity).map(([grade, config]) => [grade, (config as GradeSensitivityConfig).factor])
)

/** 区域竞争强度 */
export const DISTRICT_COMPETITION: Record<string, DistrictCompetitionConfig> = DistrictCompetition as Record<string, DistrictCompetitionConfig>

/** 风险因素配置 */
export const RISK_FACTORS = RiskFactors

/** 转化话术 */
export const CONVERSION_COPIES: Record<FeasibilityLevel, ConversionCopyConfig> = ConversionCopy as Record<FeasibilityLevel, ConversionCopyConfig>
 ...
 * 获取 Band 等级配置
 */
export function getBandConfig(band: 1 | 2 | 3): BandDifficultyConfig {
 ...
 * 获取年级难度系数
 */
export function getGradeFactor(grade: string): number {
 ...
 * 获取区域竞争系数
 */
export function getDistrictFactor(district: string): number {
 ...
 * 获取转化话术
 */
export function getConversionCopy(level: FeasibilityLevel): ConversionCopyConfig {
 ...
 * @returns 可行性等级 A-E
 */
export function scoreToLevel(score: number): FeasibilityLevel {
 ...
 * 获取等级对应的分数范围描述
 */
export function getLevelScoreRange(level: FeasibilityLevel): string {
  switch (level) {
 ...
 * 可行性等级描述
 */
export const LEVEL_DESCRIPTIONS: Record<FeasibilityLevel, string> = {
...
/** 根据等级获取推荐行动 */
export const RECOMMENDED_ACTIONS: Record<FeasibilityLevel, LevelRecommendation> = 
  RecommendedActions as Record<FeasibilityLevel, LevelRecommendation>

/**
 * 获取等级对应的推荐行动
 */
export function getRecommendedActions(level: FeasibilityLevel): LevelRecommendation {
 ...
 * 获取主要推荐行动（优先级最高的）
 */
export function getPrimaryAction(level: FeasibilityLevel): RecommendedAction {
