...
export interface LearningEventInput {
  ...
  accuracy?: number;       // 可选，不传则自动计算
  sourceId?: string;       // 来源 ID（如 quiz_session_id）
 ...
 * 批量记录学习事件
 */
export async function recordLearningEventsBatch(
  db: D1Database,
...
export async function getUserLearningStats(
  ...
  // 构建分科目统计对象
  const subjectStats: LearningStats['subjectStats'] = {};
 ...
 * 获取今日学习统计
 */
export async function getTodayLearningStats(
  db: D1Database,
  userId: string
): Promise<{
  quizCount: number;
...
export async function getUserStreakDays(
  ...
  const result = await db.prepare(`
    ...
    LIMIT 60
  `).bind(userId).all<{ learning_date: string }>();
 ...
 */
export async function getLeaderboardFromEvents(
  db: D1Database,
  options: {
 ...
 */
export async function getUserRankFromEvents(
  db: D1Database,
  userId: string,
  options: {
    metric: 'QUIZ_COUNT' | 'ACCURACY' | 'SPEED';
