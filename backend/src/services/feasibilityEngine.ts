...
export interface FeasibilityResult {
  ...
  improvementPlan: string[] // 3-6个月可执行建议
  subjectAnalysis: SubjectAnalysisResult[]
  preparationPlan: PreparationPlan
  conversionCopy: ConversionCopy // 转化话术
  riskRadar: RiskRadarItem[] // 风险雷达
  disclaimer: string
}
...
const BAND_RULES = {
  1: {
    ...
    englishIntensity: 'high' as const
  },
  2: {
    minEnglish: 65,
    minMath: 60,
    ...
    englishIntensity: 'medium' as const
  },
  3: {
    minEnglish: 55,
    minMath: 55,
...
/** 区域竞争强度系数 */
const DISTRICT_COMPETITION: Record<string, { level: string; factor: number }> = {
...
const CONVERSION_COPIES: Record<FeasibilityLevel, ConversionCopy> = {
  'A': {
...
function analyzeStudentAbility(student: StudentProfile): {
  ...
  const getScore = (keys: string[]) => {
    for (const k of keys) {
      ...
      if (normalized) return scores[normalized]
    }
    return 0
  }
...
function generateSubjectAnalysis(
  ...
  for (const [subject, score] of Object.entries(student.scores)) {
    ...
    if (lowerSubject === 'english') threshold = bandRules.minEnglish
    if (lowerSubject === 'chinese') threshold = bandRules.minChinese
    if (lowerSubject === 'math') threshold = bandRules.minMath
...
function generateSummary(
  ...
  const summaries: Record<FeasibilityLevel, (a: typeof ability) => string> = {
    ...
    if (content) {
      ...
      if (jsonMatch) {
        ...
        if (aiResult.mainRisks && Array.isArray(aiResult.mainRisks)) {
          ...
          aiResult.mainRisks.forEach((r: string) => {
