...
export const QUESTION_TYPE_RULES: Record<string, QuestionTypeRules> = {
  multiple_choice: {
    requiredPrecision: 0,
    unitRequired: false,
...
export class NumericMatcher {
   ...
   * 数值相等性判断（带容差）
   */
  isNumericEqual(userNum: number, expectedNum: number, precision: number = 0.001): boolean {
    if (expectedNum === 0) {
   ...
   * 匹配数值
   */
  match(userAnswer: string, expectedAnswer: string, precision: number = 0.001): boolean {
...
export class EquationMatcher {
  private numericMatcher = new NumericMatcher()
...
export class UnitProcessor {
   ...
   * 提取数值和单位
   */
  extractValueAndUnit(text: string): { value: number; unit: string } | null {
...
export class IntelligentAnswerMatcher {
  private preprocessor = new AnswerPreprocessor()
  private numericMatcher = new NumericMatcher()
  private equationMatcher = new EquationMatcher()
  private unitProcessor = new UnitProcessor()
  ...
  async matchAnswer(
    ...
    if (this.normalizedMatch(normalizedUser, normalizedExpected)) {
      return this.createResult(true, 'normalized', userAnswer, expectedAnswer, 0.95, 
        '答案正确！（系统已自动识别您的答案格式）✅')
    }

    // 第3层：数值匹配（针对计算题）
    if (rules.acceptPlainNumber && this.numericMatcher.match(normalizedUser, normalizedExpected, rules.requiredPrecision)) {
      return this.createResult(true, 'numeric', userAnswer, expectedAnswer, 0.9,
        '答案正确！（数值匹配）✅')
    }

    // 第4层：方程匹配
    if (rules.acceptEquationForm && this.equationMatcher.match(normalizedUser, normalizedExpected)) {
      return this.createResult(true, 'equation', userAnswer, expectedAnswer, 0.9,
        '答案正确！（系统已识别方程解）✅')
    }
   ...
   * 完全匹配
   */
  private exactMatch(userAnswer: string, expectedAnswer: string): boolean {
   ...
   * 规范化后匹配
   */
  private normalizedMatch(normalizedUser: string, normalizedExpected: string): boolean {
    // 忽略大小写比较
   ...
   * 选择题匹配
   */
  private matchChoice(userChoice: string, expectedChoice: string): boolean {
    // 只比较第一个字符（选项字母）
  ...
  private createResult(
    isCorrect: boolean,
    matchType: AnswerMatchingResult['matchType'],
    userAnswer: string,
    expectedAnswer: string,
    confidence: number,
    feedback: string
