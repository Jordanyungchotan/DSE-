...
import {
  ...
  CONVERSION_COPIES,
  DISCLAIMER,
  type BandDifficultyConfig,
  type FeasibilityLevel,
  type ConversionCopyConfig,
 ...
 * 获取科目分数
 */
function getSubjectScore(scores: Record<string, number>, subjectKeys: string[]): number {
  for (const key of subjectKeys) {
    if (scores[key] !== undefined) return scores[key]
    // 尝试标准化后匹配
    const normalized = normalizeSubjectName(key)
    for (const [k, v] of Object.entries(scores)) {
      if (normalizeSubjectName(k) === normalized) return v
    }
  }
  return 0
}
 ...
 * 判断科目状态
 */
function getSubjectStatus(score: number, required: number): 'strong' | 'adequate' | 'weak' | 'critical' {
  const gap = score - required
  if (gap >= 10) return 'strong'
  if (gap >= 0) return 'adequate'
  if (gap >= -10) return 'weak'
  return 'critical'
}
...
): Array<PlacementScoreResult & { schoolName: string }> {
 ...
 * 获取最佳匹配学校
 */
export function getBestMatchSchool(
  student: StudentProfile,
  targetSchools: TargetSchool[]
): { school: TargetSchool; result: PlacementScoreResult } | null {
 ...
 * - < 30 → E
 */
export function getSimpleScore(
  student: StudentProfile,
  targetSchool: TargetSchool
): SimpleScoreResult {
 ...
 * 批量获取简化评分
 */
export function getMultipleSimpleScores(
  student: StudentProfile,
  targetSchools: TargetSchool[]
): Array<SimpleScoreResult & { schoolName: string }> {
  return targetSchools.map(school => {
