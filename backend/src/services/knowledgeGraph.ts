...
export interface KnowledgeNode {
  ...
  metadata: {
    description?: string
    difficultyBaseline?: number  // 基础难度 1-5
    prerequisites?: string[]     // 前置知识点ID
    commonMisconceptions?: string[]  // 常见错误概念
    examFrequency?: number       // DSE考试频率 1-5
    teachingOrder?: number       // 教学顺序
  }
  createdAt: string
  updatedAt: string
}
...
export interface CoverageScore {
  conceptualCoverage: {
    ...
    coverageDepth: number  // 1-浅层 2-中层 3-深层
  }
  skillCoverage: {
...
// 数学科知识点
export const MATH_KNOWLEDGE_TREE: Omit<KnowledgeNode, 'id' | 'createdAt' | 'updatedAt'>[] = [
  // 一级主题
...
// 物理科知识点
export const PHYSICS_KNOWLEDGE_TREE: Omit<KnowledgeNode, 'id' | 'createdAt' | 'updatedAt'>[] = [
  // 一级主题
...
// 化学科知识点
export const CHEMISTRY_KNOWLEDGE_TREE: Omit<KnowledgeNode, 'id' | 'createdAt' | 'updatedAt'>[] = [
  // 一级主题
...
export class KnowledgeGraphManager {
  ...
  private nodesBySubject: Map<string, KnowledgeNode[]>
  private nodesByType: Map<string, KnowledgeNode[]>
   ...
   * 生成节点ID
   */
  private generateNodeId(subject: string, name: string): string {
   ...
   * 获取相关概念
   */
  getRelatedConcepts(conceptId: string, depth: number = 2): KnowledgeNode[] {
  ...
  analyzeQuestionCoverage(question: {
    ...
    // 计算覆盖深度
    let coverageDepth = 1
    if (primaryConcepts.length >= 3) coverageDepth = 3
    else if (primaryConcepts.length >= 2) coverageDepth = 2
   ...
   * 提取认知技能
   */
  private extractCognitiveSkills(text: string): string[] {
    const skills: string[] = []
   ...
   * 提取过程技能
   */
  private extractProceduralSkills(text: string, subject: string): string[] {
    const skills: string[] = []
    
    const skillPatterns: Record<string, Array<{ skill: string, regex: RegExp }>> = {
      math: [
   ...
   * 建立默认关系
   */
  private buildDefaultRelationships(): void {
    const now = new Date().toISOString()
    let relId = 0
    ...
    const mathRelations = [
      // 因数与倍数相关
      { source: 'math_因数与倍数', target: 'math_最大公因数hcf', type: 'leads_to' as const, strength: 0.9 },
      { source: 'math_因数与倍数', target: 'math_最小公倍数lcm', type: 'leads_to' as const, strength: 0.9 },
      { source: 'math_因数与倍数', target: 'math_质因数分解', type: 'leads_to' as const, strength: 0.8 },
      { source: 'math_质因数分解', target: 'math_最大公因数hcf', type: 'related_to' as const, strength: 0.9 },
      { source: 'math_质因数分解', target: 'math_最小公倍数lcm', type: 'related_to' as const, strength: 0.9 },
      
      // 代数相关
      { source: 'math_代数表达式', target: 'math_多项式', type: 'leads_to' as const, strength: 0.8 },
      { source: 'math_多项式', target: 'math_方程', type: 'leads_to' as const, strength: 0.7 },
      { source: 'math_一元一次方程', target: 'math_二元一次方程组', type: 'prerequisite' as const, strength: 0.9 },
      { source: 'math_一元一次方程', target: 'math_一元二次方程', type: 'prerequisite' as const, strength: 0.8 },
      
      // 几何相关
      { source: 'math_平面几何', target: 'math_三角形', type: 'part_of' as const, strength: 0.9 },
      { source: 'math_平面几何', target: 'math_四边形', type: 'part_of' as const, strength: 0.9 },
      { source: 'math_平面几何', target: 'math_圆', type: 'part_of' as const, strength: 0.9 },
      { source: 'math_三角形', target: 'math_三角学', type: 'leads_to' as const, strength: 0.8 },
      
      // 技能关系
      { source: 'math_方程', target: 'math_代数操作', type: 'requires_skill' as const, strength: 0.9 },
      { source: 'math_三角形', target: 'math_几何推理', type: 'requires_skill' as const, strength: 0.8 },
    ]
...
export function getKnowledgeGraph(): KnowledgeGraphManager {
