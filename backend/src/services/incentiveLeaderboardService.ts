...
export async function getIncentiveLeaderboard(
  ...
  // 处理排名和特权
  const entries: IncentiveLeaderboardEntry[] = [];
  let currentRank = 1;
  ...
  // 获取当前用户排名
  let myRank: IncentiveMyRankInfo | undefined;
  if (currentUserId) {
    ...
    if (userInEntries) {
      ...
      // 计算与前一名的差距
      let gapToNext: IncentiveMyRankInfo['gapToNext'];
 ...
 * 获取用户的激励排名
 */
async function getUserIncentiveRank(
  db: D1Database,
  userId: string,
  type: IncentiveLeaderboardType,
  totalParticipants: number
): Promise<IncentiveMyRankInfo | null> {
  // 获取用户积分
  const userQuery = `
    SELECT 
      ups.user_id,
  ...
  const rankResult = type === 'POINTS_WEEKLY'
    ? await db.prepare(rankQuery).bind(userId).first<{ higher_count: number }>()
    : await db.prepare(rankQuery).bind(userResult.total_points).first<{ higher_count: number }>();
